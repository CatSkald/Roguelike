...
$NUGET_PATH install NUnit.ConsoleRunner -Version 3.7.0 -OutputDirectory packages
$NUGET_PATH install OpenCover -Version 4.6.519 -OutputDirectory packages
packages/FAKE/tools/FAKE.exe $@ --fsiargs -d:MONO build.fsx
else
mono $NUGET_PATH install FAKE -OutputDirectory packages -ExcludeVersion
mono $NUGET_PATH install NUnit.ConsoleRunner -Version 3.7.0 -OutputDirectory packages
